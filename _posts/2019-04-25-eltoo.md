...
Copied from comments on this post (2019-2020) via Disqus.

- Tejaswi Nadahalli, 2019-04-25.
  I think you should point out the potential dangers of the SIGHASH_NOINPUT flag. It's called out in the BIP, but I think it's worth mentioning again. And interestingly, it's completely unrelated to L2 or related, and is about replay possibilities outside the control of the SIGHASH_NOINPUT user. I think that's kind of interesting.
  - Sergei Tikhomirov, 2019-04-25.
    Thanks, I'll take a look! Is this some kind of "transaction malleability 2.0"?
    - Richard Myers, 2019-04-26.
      The lightning mailing list (May 2018) has some discussion about the down sides of SIGHASH_NOINPUT.
      I agree with the argument that it's no worse than SIGHASH_NONE and will only be used by specialized scripts. Best summed up by Rusty: "If I can convince you to sign with SIGHASH_NONE, it's already a problem today."
      I see the arguments against SIGHASH_NOINPUT as mostly bike-shedding.
      - Tejaswi Nadahalli, 2019-04-26.
        I don't agree 100%.
        SIGHASH_NONE (even with ANYONE_CAN_PAY) user knows that the particular transaction's outputs are going to the miner, most likely. SIGHASH_NOINPUT user, on the other hand, needs to ensure that he never gets paid to a public key hash (or P2SH or what have you) twice if he ever uses the first of those payments in a SIGHASH_NOINPUT transaction. Unlike SIGHASH_NONE, which just burns a specific transaction, SIGHASH_NOINPUT burns the address/script. So, if I can convince you to sign with SIGHASH_NOINPUT, more than 1 transaction is potentially vulnerable, unlike SIGHASH_NONE, where just 1 transaction is vulnerable.
        The benefits of SIGHASH_NOINPUT are eltoo, of course, and some Lightning Watchtower space storage maybe.
        IMHO, these are legit trade-offs, and not just bike-shedding.
- Richard Myers, 2019-04-26.
  Thank you for this fantastic summary. I had to read the paper multiple times before I could really get what was going on. Your description boils down the essential points very well.
- Andrej Mitrovic, 2020-10-16.
  Hey Sergei. The whitepaper (which might have been updated?) now describes the use of a trigger transaction, which resolves one of the drawbacks of the initial design with regards to channel expiration. Have you taken a look at it?
  - Sergei Tikhomirov, 2020-10-16.
    Thank you for the update, I haven't looked at it yet! Is this - <https://blockstream.com/eltoo.pdf> - the updated version?